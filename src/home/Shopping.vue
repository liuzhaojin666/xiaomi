<template>
    <div>
    	<hear num='购物车'></hear>
		<div class="deng">
	       	<h3>登录后享受更多优惠<span>去登陆<van-icon name="arrow" /></span></h3>
	       	<div class="nr" v-if="this.a==1">
	       		<p>购物车还是空的<button>去逛逛</button></p>
	    	</div>
	    	<div class="shuju" v-if="this.a==2">
	    		<ul>
    	...
    	<div class="sj">
    		<ul>
    			<li v-for="(item,index) in arr" @click="fn(index)" :key="index">
    				<img :src="item.img">
    				<div class="xia">
    					<h2>{{ item.name }}</h2>
    					<p>{{ item.content }}</p>
